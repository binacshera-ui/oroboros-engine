...
pub struct WorldManager {
    ...
    /// Chunks currently being generated by worker threads.
...
impl WorldManager {
    ...
    /// Processes the generation queue using async worker thread.
    /// 
    /// NON-BLOCKING: Sends work to worker, receives completed chunks.
    ...
    pub fn flush_generation_queue(&mut self) {
        // First, drain any in-flight results
        while let Ok(result) = self.result_receiver.try_recv() {
            self.in_flight.remove(&result.coord);
        ...
        // Then generate remaining queued chunks synchronously
